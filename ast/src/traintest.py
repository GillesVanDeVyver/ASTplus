...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        if best_epoch == epoch:
            torch.save(audio_model.state_dict(), "%s/ast_model/best_audio_model.pth" % (exp_dir))
            torch.save(optimizer.state_dict(), "%s/ast_model/best_optim_state.pth" % (exp_dir))

        torch.save(audio_model.state_dict(), "%s/ast_model/audio_model.%d.pth" % (exp_dir, epoch))
        if len(train_loader.dataset) > 2e5:
            torch.save(optimizer.state_dict(), "%s/ast_model/optim_state.%d.pth" % (exp_dir, epoch))
...
def validate_wa(audio_model, val_loader, args, start_epoch, end_epoch):
    ...
    sdA = torch.load(exp_dir + '/ast_model/audio_model.' + str(start_epoch) + '.pth', map_location=device)

    model_cnt = 1
    for epoch in range(start_epoch+1, end_epoch+1):
        sdB = torch.load(exp_dir + '/ast_model/audio_model.' + str(epoch) + '.pth', map_location=device)
        for key in sdA:
            sdA[key] = sdA[key] + sdB[key]
        model_cnt += 1

        # if choose not to save ast_model of epoch, remove to save space
        if args.save_model == False:
            os.remove(exp_dir + '/ast_model/audio_model.' + str(epoch) + '.pth')
    ...
    torch.save(audio_model.state_dict(), exp_dir + '/ast_model/audio_model_wa.pth')
